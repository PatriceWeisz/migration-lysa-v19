#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""MIGRATION ÉTIQUETTES CONTACT"""
import sys, os, json
from pathlib import Path

# AFFICHER IMMÉDIATEMENT
print("="*70)
print("DEMARRAGE: MIGRATION ETIQUETTES CONTACT")
print("="*70)
...
LOGS_DIR = Path('logs')
mapping_file = LOGS_DIR / 'partner_category_mapping.json'
mapping = json.load(open(mapping_file)) if mapping_file.exists() else {}
mapping = {int(k): v for k, v in mapping.items()}
print(f"Mapping: {len(mapping)}")

src = conn.executer_source('res.partner.category', 'search_read', [],
                           fields=['name', 'color', 'parent_id', 'active'])
print(f"SOURCE: {len(src)}")

dst = conn.executer_destination('res.partner.category', 'search_read', [], fields=['name'])
...
for idx, rec in enumerate(src, 1):
    ...
    try:
        ...
        dest_id = conn.executer_destination('res.partner.category', 'create', data)
