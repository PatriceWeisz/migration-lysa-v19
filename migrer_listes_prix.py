...
LOGS_DIR = Path('logs')
mapping_file = LOGS_DIR / 'pricelist_mapping.json'
mapping = json.load(open(mapping_file)) if mapping_file.exists() else {}
mapping = {int(k): v for k, v in mapping.items()}
print(f"Mapping: {len(mapping)}")

src = conn.executer_source('product.pricelist', 'search_read', [],
                           fields=['name', 'currency_id', 'company_id', 'active'])
print(f"SOURCE: {len(src)}")

dst = conn.executer_destination('product.pricelist', 'search_read', [], fields=['name'])
...
for idx, rec in enumerate(src, 1):
    ...
    try:
        ...
        dest_id = conn.executer_destination('product.pricelist', 'create', data)
